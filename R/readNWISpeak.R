...
#' @note Peak flow data do not always have complete date information due to uncertainity 
#'in the exact day of the peak. The column \code{peak_dt} is always type "character" but 
#'can be converted by the user.
#' @seealso \code{\link{importRDB}}
#' @keywords manip IO
#' @export
#' @examples
#'\dontrun{
#'# Get the peaks for USGS station identifier
#'# 01578310 SUSQUEHANNA RIVER AT CONOWINGO, MD
#'readNWISpeak("01578310")
#'}
readNWISpeak <- function(site, convert.type=TRUE) {
  ## Coding history:
  ...
  ##    2014Jun23 DLLorenz made specific to peak data
  ##
    myurl <- url(paste("http://nwis.waterdata.usgs.gov/usa/nwis/peak/?site_no=", site,
                       "&range_selection=date_range&format=rdb", sep=""))
